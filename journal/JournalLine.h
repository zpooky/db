...
JournalLine<hash_t> segment_line(journal::id p_id,
                                 ...
                                 db::segment::id seg_id, State p_state) {
  hash_t h;
  h.update(p_id);
  h.update(p_table);
  h.update(seg_id);
...
JournalLine<hash_t> empty_segment_line() {
  using name_type = db::table::name::type;
  name_type name{0};
  return segment_line<hash_t>(journal::NO_ID, name, db::segment::NO_ID,
                              State::INTERNAL);
}
...
JournalLine<hash_t> xbegin(journal::id jid,
  ...
  return segment_line<hash_t>(jid, table, db::segment::NO_ID, State::BEGIN);
}
